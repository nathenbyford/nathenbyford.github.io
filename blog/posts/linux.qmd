...
- `rstan` or `rjags`: For Bayesian statistics.

### Bayesian package setup

To get `rstan` to work on any system you need to install `rtools` and other dependencies. This is no different on linux. The `rstan` [wiki/setup page](https://github.com/stan-dev/rstan/wiki/Configuring-C-Toolchain-for-Linux) has everything needed on there.

Essentially, run the following lines for Ubuntu LTS systems and `rstan` should be up and running.
...
For `rjags`, JAGS will need to installed separatly from R. To do so it can be downloaded from their [sourceforge page](https://mcmc-jags.sourceforge.io/), note that if you are not using Ubuntu or Debian, JAGS may not be available unless compiled from source.
